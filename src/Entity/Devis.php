...
class Devis
{
    ...
    #[ORM\Column(type: Types::DECIMAL, precision: 10, scale: 2, nullable: true)]
    private ?string $prixArrhes = null;
    ...
    #[ORM\OneToMany(targetEntity: Client::class, mappedBy: 'devis', cascade: ['persist', 'remove'], orphanRemoval: true)]
    private Collection $clients;

    /** @var Collection<int, DevisKit> */
    #[ORM\OneToMany(targetEntity: DevisKit::class, mappedBy: 'devis', cascade: ['persist', 'remove'], orphanRemoval: true)]
    ...
    public function getPrixArrhes(): ?string
    {
        return $this->prixArrhes;
    }

    public function setPrixArrhes(?string $prixArrhes): static
    {
        $this->prixArrhes = $prixArrhes;
    ...
    public function removeClient(Client $client): static
    {
        if ($this->clients->removeElement($client)) {
            if ($client->getDevis() === $this) {
                $client->setDevis(null);
            }
        }
    ...
    /**
     * @return Collection<int, DevisKit>
     */
    ...
    public function addLigneKit(DevisKit $ligneKit): static
    {
        if (!$this->lignesKits->contains($ligneKit)) {
            $this->lignesKits->add($ligneKit);
            $ligneKit->setDevis($this);
        }

        return $this;
    }

    public function removeLigneKit(DevisKit $ligneKit): static
    {
        if ($this->lignesKits->removeElement($ligneKit)) {
            if ($ligneKit->getDevis() === $this) {
                $ligneKit->setDevis(null);
            }
        }
