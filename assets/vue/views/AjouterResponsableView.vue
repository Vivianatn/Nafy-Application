  ...
  <div class="anim-page">
    ...
    <form class="form anim-form" @submit.prevent="envoyer">
      ...
      <button type="submit" class="bouton bouton--bloc" :disabled="envoiEnCours">
        {{ envoiEnCours ? 'Envoi…' : 'Ajouter un responsable' }}
      </button>
    </form>
  </div>
</template>
...
async function envoyer() {
  ...
  try {
    const reponse = await api.post('/responsables', form)

    const responsable = reponse.data
    reinitialiser()
    notifier(`Le responsable « ${responsable.prenom} ${responsable.nom} » a bien été ajouté.`, 'succes')
